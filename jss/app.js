...
// VERSÃO DASHBOARD EXECUTIVO: KPIs, Gráficos Multimatéria e Lógica de Negócio Sênior
const App = {
    state: {
        userId: null,
        db: null, 
...
RETORNE JSON ESTRITO: { "resumo_executivo": "...", "analise_psicopedagogica": "...", "diagnostico_kumon": {}, "plano_acao_imediato": [], "requer_validacao_humana": true }`;
        ...
        this.elements.studentList.innerHTML = list.length ? list.map(([id, s]) => `
            <div class="student-card" onclick="App.openStudentModal('${id}')">
                ...
                <div class="student-stages">
                    ${s.mathStage?`<span class="stage-item" style="border-left:3px solid #0078c1">Mat: ${s.mathStage}</span>`:''}
                    ${s.portStage?`<span class="stage-item" style="border-left:3px solid #d62828">Port: ${s.portStage}</span>`:''}
                    ${s.engStage?`<span class="stage-item" style="border-left:3px solid #f59e0b">Ing: ${s.engStage}</span>`:''}
                </div>
    ...
    getSubjectBadge(subject) {
        if (!subject) return '';
        let color = '#888';
        if (subject === 'Matemática') color = '#0078c1'; 
        if (subject === 'Português') color = '#d62828'; 
        if (subject === 'Inglês') color = '#f59e0b'; 
